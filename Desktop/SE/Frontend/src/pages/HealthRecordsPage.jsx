...
const SEED_CONDITIONS = [
    { id: 1, condition: 'Type 2 Diabetes', diagnosedDate: 'Jan 2020', status: 'Ongoing', icon: '🩸', notes: 'Managed with Metformin 500mg twice daily.' },
    { id: 2, condition: 'Hypertension', diagnosedDate: 'Mar 2019', status: 'Ongoing', icon: '💓', notes: 'Diet-controlled + Amlodipine 5mg.' },
    { id: 3, condition: 'Seasonal Allergy', diagnosedDate: 'Apr 2022', status: 'Resolved', icon: '🤧', notes: 'Resolved with antihistamine course.' },
];
...
const HealthRecordsPage = () => {
    ...
    const [conditions, setConditions] = useState(SEED_CONDITIONS);
    const [prescriptions] = useState(SEED_PRESCRIPTIONS);
    ...
    // Summary stats
    const stats = [
        { label: 'Active Conditions', value: conditions.filter(c => c.status === 'Ongoing').length, icon: '💓', color: 'bg-red-50 text-red-600' },
                        ...
                        {activeTab === 'history' && (
                            <div className="space-y-4 animate-[fadeIn_0.3s_ease-out_both]">
                                ...
                                {conditions.length === 0 ? (
                                    ...
                                    conditions.map(c => (
                                        <div key={c.id} className="group flex items-start gap-5 p-5 rounded-2xl border border-gray-100 hover:border-gray-200 hover:bg-gray-50/50 transition-all">
                                            ...
                                            <div className="flex-1 min-w-0">
                                                <div className="flex items-start justify-between gap-3 flex-wrap">
                                                    <h3 className="font-bold text-gray-900 text-base group-hover:text-[#01579B] transition-colors">{c.condition}</h3>
                                                    <StatusBadge status={c.status} />
                                                </div>
                                                <p className="text-xs text-gray-400 mt-1 font-semibold">Diagnosed: {c.diagnosedDate}</p>
                                                <p className="text-sm text-gray-500 mt-2 leading-relaxed">{c.notes}</p>
                                            </div>
                                        </div>
                                    ))
                                )}
                                            ...
                                            >
                                                <div className="flex items-center gap-4">
                                                    <div className="w-11 h-11 rounded-xl bg-purple-50 text-purple-600 flex items-center justify-center font-black text-sm shrink-0">
                                                        {p.doctor.split(' ').slice(1).map(n => n[0]).join('')}
                                                    </div>
                                                    <div className="text-left">
                                                        <h3 className="font-bold text-gray-900 text-sm">{p.doctor}</h3>
                                                        <p className="text-xs text-gray-500">{p.specialty} • {p.date}</p>
                                                    </div>
                                                </div>
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <svg className={`w-4 h-4 text-gray-400 transition-transform ${expandedPrescription === p.id ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 9l-7 7-7-7" /></svg>
                                                </div>
                                            </button>
                                                ...
                                                <div className="border-t border-gray-100 p-5 bg-gray-50 space-y-3 animate-[fadeIn_0.2s_ease-out_both]">
                                                    {p.medicines.map((m, i) => (
                                                        <div key={i} className="flex items-center justify-between p-4 bg-white rounded-xl border border-gray-100">
                                                            ...
                                                            <span className="text-sm font-black text-[#028090] bg-[#f0fdfc] px-3 py-1.5 rounded-lg border border-[#028090]/20">{m.dose}</span>
                                                        </div>
                                                    ))}
                                                </div>
                                            )}
