...
import { mockPatients, mockMedicalRecords, mockPrescriptions } from '../data/mockData';
...
const PatientDetails = () => {
    ...
    useEffect(() => {
        const found = mockPatients.find(p => p.id === parseInt(id));
        if (found) {
            setPatient(found);
            setRecords(mockMedicalRecords.filter(r => r.patientId === found.id));
            setPrescriptions(mockPrescriptions.filter(p => p.patientId === found.id));
        }
    }, [id]);
    ...
    if (!patient) return <div className="p-8 text-center text-gray-500">Loading patient...</div>;
            ...
            <div className="space-y-4">
                {activeTab === 'history' && (
                    records.length > 0 ? records.map(record => (
                        <div key={record.id} className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
                            <div className="flex justify-between items-start mb-3">
                                ...
                                <span className={`text-xs font-bold px-3 py-1 rounded-full border ${record.status === 'Resolved' ? 'bg-green-50 text-green-700 border-green-200' : 'bg-amber-50 text-amber-700 border-amber-200'}`}>
                                    {record.status}
                                </span>
                            </div>
                            <p className="text-gray-600 text-sm mt-3 leading-relaxed">{record.description}</p>
                            <p className="text-xs text-gray-400 font-semibold mt-4">{record.date}</p>
                        </div>
                ...
                {activeTab === 'prescriptions' && (
                    prescriptions.length > 0 ? prescriptions.map(pres => (
                        <div key={pres.id} className="bg-white rounded-2xl p-5 border border-indigo-50 shadow-sm flex items-start gap-4">
                            ...
                            <div className="flex-1">
                                <h3 className="font-bold text-gray-800">{pres.medicine}</h3>
                                ...
                                <p className="text-xs text-gray-400 font-semibold mt-3">{pres.date}</p>
                            </div>
                        </div>
