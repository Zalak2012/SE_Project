...
const PaymentPage = () => {
    ...
    const initPayment = async () => {
        ...
        try {
            ...
            const options = {
                key: "rzp_test_ShOSBAQY65rTHB", // Can also be loaded from env
    ...
    const handleConfirmBooking = async (methodOverride = null) => {
        ...
        if (finalMethod === "online" && !methodOverride) {
            return initPayment();
        }
                        ...
                        <div className="bg-white rounded-3xl p-6 md:p-8 border border-gray-100 shadow-sm">
                            ...
                            <div className="space-y-4">
                                ...
                                <label className={`relative flex items-start sm:items-center p-5 rounded-2xl border-2 cursor-pointer transition-colors ${paymentMethod === 'cash' ? 'border-[#00A896] bg-[#00A896]/5' : 'border-gray-200 bg-white hover:bg-gray-50'}`}>
                                    ...
                                    {paymentMethod === 'cash' && <CheckCircle2 className="absolute top-5 right-5 sm:relative sm:top-auto sm:right-auto w-6 h-6 text-[#00A896] ml-4 shrink-0" />}
                                </label>
                                ...
                                <label className={`relative flex items-start sm:items-center p-5 rounded-2xl border-2 cursor-pointer transition-colors ${paymentMethod === 'online' ? 'border-[#028090] bg-[#028090]/5' : 'border-gray-200 bg-white hover:bg-gray-50'}`}>
                                    <div className="flex items-center h-6">
                                        <input 
                                            ...
                                            className="w-5 h-5 text-[#028090] focus:ring-[#028090] border-gray-300"
                                        />
                                    </div>
                                    <div className="ml-4 flex-1 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                                        <div className="flex items-center gap-3">
                                            <div className="bg-white p-2 rounded-lg shadow-sm border border-gray-100">
                                                <CreditCard className={`w-6 h-6 ${paymentMethod === 'online' ? 'text-[#028090]' : 'text-gray-400'}`} />
                                            </div>
                                            <div>
                                                <span className="block font-bold text-[#1F2937]">Online Payment</span>
                                                <span className="block text-sm text-gray-500 font-medium">Pay securely via Razorpay</span>
                                            </div>
                                        </div>
                                    </div>
                                    {paymentMethod === 'online' && <CheckCircle2 className="absolute top-5 right-5 sm:relative sm:top-auto sm:right-auto w-6 h-6 text-[#028090] ml-4 shrink-0" />}
                                </label>
                            </div>
                        </div>
                    </div>
                        ...
                        <div className="bg-white rounded-3xl p-6 md:p-8 border border-gray-100 shadow-sm sticky top-8">
                            ...
                            <div className="flex gap-4 items-center mb-6 pb-6 border-b border-gray-100">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm text-[#028090] font-medium">{doctor.specialization}</p>
                            ...
                            <ul className="space-y-4 mb-6">
                                ...
                                <li className="flex justify-between items-start text-sm">
                                    ...
                                    <span className="font-bold text-gray-800 text-right">₹{doctor.consultationFee}</span>
                                </li>
                            ...
                            <div className="border-t border-gray-100 pt-5 mb-6 flex items-center justify-between">
                                ...
                                <span className="font-black text-2xl text-[#028090]">₹{doctor.consultationFee}</span>
                            </div>
