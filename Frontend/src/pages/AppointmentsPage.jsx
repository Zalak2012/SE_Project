    ...
    return (
        <div className="min-h-screen bg-gray-50 font-sans text-gray-800 flex flex-col w-full">
            ...
            <main className="flex-grow flex flex-col items-center p-4 py-12 w-full max-w-4xl mx-auto space-y-8">
                ...
                {/* ---------- APPOINTMENT LIST ---------- */}
                <section className="w-full space-y-4 animate-[slideUp_0.5s_ease-out_both] min-h-[300px]">
                    {loading ? (
                        <div className="flex flex-col items-center justify-center py-20">
                            <Loader2 className="w-10 h-10 text-[#028090] animate-spin" />
                            <p className="text-gray-400 mt-4 font-medium">Loading your appointments...</p>
                        </div>
                        ...
                        filteredAppointments.map(app => (
                            ...
                            >
                                ...
                                <div className="flex items-center gap-5">
                                    <div className="w-14 h-14 rounded-full bg-blue-50 text-[#0277BD] flex items-center justify-center font-bold text-lg border border-blue-100 group-hover:scale-110 transition-transform duration-300 overflow-hidden">
                                        {app.doctorId?.image ? (
                                            <img 
                                                src={getImageUrl(app.doctorId.image)} 
                                    ...
                                    <div className="space-y-1">
                                        <h4 className="text-lg font-bold text-gray-900 group-hover:text-[#01579B] transition-colors">{app.doctorId?.name || "CareMate Doctor"}</h4>
                                        <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-sm font-medium">
                                            <span className="text-[#028090] uppercase text-[10px] tracking-widest">{app.doctorId?.specialization || "Specialist"}</span>
                                            <span className="text-gray-300 hidden md:block">•</span>

                                            <span className="text-gray-500">
                                                {new Date(app.appointmentDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })} • {app.appointmentTime}
                                            </span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="flex items-center md:justify-end gap-4 min-w-[140px]">
                                    ...
                                    {app.status === 'completed' && !submittedFeedbacks.has(app._id) && (
                                        <button 
                ...
                <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm animate-[fadeIn_0.3s_ease-out]">
                    <div className="bg-white rounded-3xl w-full max-w-md overflow-hidden shadow-2xl animate-[slideUp_0.4s_ease-out]">
                        ...
                        <div className="p-6 border-b border-gray-100 bg-[#eaf4fb]/50">
                            ...
                            <p className="text-sm text-[#028090] font-medium mt-1">Doctor: {selectedAppt.doctorId?.name}</p>
                        </div>
