...
const FindDoctors = () => {
    ...
    React.useEffect(() => {
        const fetchDoctors = async () => {
            try {
                ...
                // Only set error for network failures or >= 500
    ...
    const filteredDoctors = doctors.filter(doctor => {
        ...
        if (appliedFilters.specialty && doctor.specialization !== appliedFilters.specialty) return false;
        if (doctor.rating < appliedFilters.minRating) return false;
        if (doctor.experience < appliedFilters.expRange) return false;
        if (doctor.consultationFee > appliedFilters.priceRange) return false;
        if (appliedFilters.verifiedOnly && !doctor.verified) return false;
        // if (appliedFilters.availability === 'Available Today' && !doctor.availableToday) return false; // Optional field
        return true;
    });
                    ...
                    <div className="w-full flex-1">
                        ...
                        {loading ? (
                            <div className="flex flex-col items-center justify-center py-24 space-y-4">
                                <span className="w-12 h-12 border-4 border-[#028090] border-t-transparent rounded-full animate-spin"></span>
                                <p className="text-gray-400 font-medium italic">Finding best doctors for you...</p>
                            </div>
                        ) : error ? (
                            <div className="bg-white rounded-2xl p-12 text-center shadow-sm border border-gray-100">
                                <span className="text-4xl block mb-4">⚠️</span>
                                <h3 className="text-xl font-bold text-gray-800 mb-2">Failed to load doctors</h3>
                                <p className="text-gray-500 mb-6">{error}</p>
                                <button onClick={() => window.location.reload()} className="px-6 py-2 bg-[#028090] text-white rounded-xl font-bold">Retry</button>
                            </div>
                        ) : (
                                ...
                                >
                                    ...
                                    <div className={`flex gap-4 items-start ${viewMode === 'grid' ? 'mb-4' : 'w-full sm:w-1/3'}`}>
                                        ...
                                        <div className="flex-grow">
                                            ...
                                            <p className="text-[#028090] font-medium text-sm mb-1">{doctor.specialization}</p>
                                            <div className="flex items-center gap-1 text-sm">
                                                <span className="text-yellow-400">★</span>
                                                <span className="font-bold text-gray-700">{doctor.rating}</span>
                                                <span className="text-gray-400">({doctor.patientsCount || 0} patients)</span>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className={`space-y-2 ${viewMode === 'grid' ? 'mb-6 flex-grow' : 'w-full sm:w-1/3'}`}>
                                        ...
                                        <div className="flex items-center gap-2 text-sm text-gray-600">
                                            <span className="text-[#039BE5]">🕒</span> {doctor.availability}
                                        </div>
                                    </div>
                                    ...
                                    <div className={`flex flex-col ${viewMode === 'grid' ? 'mt-auto' : 'w-full sm:w-1/3'}`}>
                                        <div className="flex items-center justify-between mb-4">
                                            <div className="flex items-center gap-2 text-sm text-gray-600">
                                                <span className="text-[#039BE5]">💲</span> <span className="font-bold text-[#028090]">₹{doctor.consultationFee}</span> Consultation Fee
                                            </div>
                                        </div>
                                        ...
                                        <div className="flex gap-4 pt-6 mt-auto">
                                            ...
                                            <button 
                                                onClick={(e) => { e.stopPropagation(); handleProtectedAction(e, `/booking/${doctor._id}`); }} 
                ...
                >
                    ...
                    >
                        ...
                        <div className="flex flex-col items-center flex-grow text-center">
                            ...
                            <p className="text-[#028090] font-medium text-md mb-4">{selectedDoctor.specialization}</p>
                            ...
                            <div className="flex items-center gap-2 mb-6 bg-[#F8FAFC] px-5 py-2.5 rounded-full border border-gray-100 shadow-sm">
                                ...
                                <span className="text-gray-400 text-sm leading-none">({selectedDoctor.patientsCount || 0})</span>
                            ...
                            <p className="text-gray-500 text-sm mb-8 leading-relaxed max-w-md">
                                Dr. {selectedDoctor.name.split(' ').pop()} is a highly rated {selectedDoctor.specialization?.toLowerCase()} dedicated to providing exceptional care. With over {selectedDoctor.experience} years of clinical experience, they specialize in offering comprehensive and patient-centered treatments.
                            </p>
                            ...
                            <div className="w-full flex gap-3">
                                ...
                                <button 
                                    onClick={(e) => handleProtectedAction(e, `/booking/${selectedDoctor._id}`)}
